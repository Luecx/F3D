...
template<typename TYPE, int ROWS, int COLS> class Matrix {
  ...
  public:
    ...
    template<typename T, int R, int C> Matrix(const Matrix<T, R, C>& other) {
    ...
    bool operator!=(const TTYPE& other) const { return !(*this == other); }
    ...
    template<int R = ROWS, int C = COLS> typename std::enable_if<R == C, TTYPE>::type inverse() const {
        TTYPE a = *this;
        TTYPE inv = TTYPE::eye();

        for (int i = 0; i < ROWS; ++i) {
            int pivot_row = i;
            TYPE pivot = a(pivot_row, i);
            TYPE abs_pivot = std::abs(pivot);
            for (int r = i + 1; r < ROWS && abs_pivot < static_cast<TYPE>(1e-8); ++r) {
                TYPE candidate = std::abs(a(r, i));
                if (candidate > abs_pivot) {
                    pivot_row = r;
                    pivot = a(r, i);
                    abs_pivot = candidate;
                }
            ...
            if (std::abs(pivot) < static_cast<TYPE>(1e-8)) {
                throw std::runtime_error("Matrix inversion failed: singular matrix");
            }

            if (pivot_row != i) {
                for (int c = 0; c < COLS; ++c) {
                    std::swap(a(i, c), a(pivot_row, c));
                    std::swap(inv(i, c), inv(pivot_row, c));
                }
            }

            TYPE inv_pivot = static_cast<TYPE>(1) / a(i, i);
            for (int c = 0; c < COLS; ++c) {
                a(i, c) *= inv_pivot;
                inv(i, c) *= inv_pivot;
            }
    ...
    template<int R = ROWS, int C = COLS> typename std::enable_if<R == 1 || C == 1, TYPE>::type length() const {
        TYPE result = 0;
        for (int i = 0; i < ROWS * COLS; ++i) {
            result += data[i] * data[i];
        }
    ...
    template<int R = ROWS, int C = COLS> typename std::enable_if<R == 1 || C == 1, TTYPE>::type normalised() const {
        TYPE len = length();
        TTYPE result = *this;
        for (int i = 0; i < ROWS * COLS; ++i) {
            result.data[i] /= len;
    ...
    template<int R = ROWS, int C = COLS> typename std::enable_if<R == 1 || C == 1, TTYPE>::type& normalise() {
        TYPE len = length();
        for (int i = 0; i < ROWS * COLS; ++i) {
            data[i] /= len;
        }
    ...
    // expose raw pointer for interfacing with APIs expecting contiguous floats
    TYPE* value_ptr() { return data.data(); }
    const TYPE* value_ptr() const { return data.data(); }

    // access operator
    TYPE& operator()(int row, int col) { return data[row * COLS + col]; }
    // access operator in case one dimension is 1
    TYPE& operator()(int index) { return data[index]; }
    // const access operator
    const TYPE& operator()(int row, int col) const { return data[row * COLS + col]; }
    ...
    typename std::enable_if<R == 3 && C == 3, TTYPE>::type static rot_x(TYPE angle) {
        TTYPE result{{1, 0, 0, 0, std::cos(angle), -std::sin(angle), 0, std::sin(angle), std::cos(angle)}};
        return result;
    }
    template<int R = ROWS, int C = COLS>
    typename std::enable_if<R == 3 && C == 3, TTYPE>::type static rot_y(TYPE angle) {
        TTYPE result{{std::cos(angle), 0, std::sin(angle), 0, 1, 0, -std::sin(angle), 0, std::cos(angle)}};
        return result;
    }
    template<int R = ROWS, int C = COLS>
    typename std::enable_if<R == 3 && C == 3, TTYPE>::type static rot_z(TYPE angle) {
        TTYPE result{{std::cos(angle), -std::sin(angle), 0, std::sin(angle), std::cos(angle), 0, 0, 0, 1}};
        return result;
    }

    // for square matrices, enable identity function as static function
    template<int R = ROWS, int C = COLS> typename std::enable_if<R == C, TTYPE>::type static eye() {
        TTYPE result;
        for (int i = 0; i < ROWS; ++i) {
            result(i, i) = 1;
        }
    ...
    template<int R, int C> TYPE dot(const Matrix<TYPE, R, C>& other) const {
        TYPE result = 0;
        for (int i = 0; i < ROWS * COLS; ++i) {
    ...
    template<int R, int C> Matrix<TYPE, ROWS, C> matmul(const Matrix<TYPE, R, C>& other) const {
    ...
    view_orthogonal(TYPE left, TYPE right, TYPE bottom, TYPE top, TYPE near, TYPE far);

    template<int R = ROWS, int C = COLS>
    typename std::enable_if<R == 4 && C == 4, Matrix<TYPE, ROWS, COLS>>::type
    view_perspective(TYPE fov, TYPE aspectRatio, TYPE near, TYPE far);

    template<int R = ROWS, int C = COLS>
    typename std::enable_if<R == 4 && C == 4, Matrix<TYPE, ROWS, COLS>>::type
    view_look_at(const Matrix<TYPE, 3, 1>& eye, const Matrix<TYPE, 3, 1>& center, const Matrix<TYPE, 3, 1>& up);
};

// definitions of some implementations
#include "mat_project.ipp"
#include "mat_transform.ipp"
