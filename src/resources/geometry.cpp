...
void calculateTangents(Vertex& v1, Vertex& v2, Vertex& v3, std::vector<Vec2f>& textureCoords) {
    Vec2f uv1       = textureCoords[v1.textureIndex];
    Vec2f uv2       = textureCoords[v2.textureIndex];
    Vec2f uv3       = textureCoords[v3.textureIndex];

    Vec3f deltaPos2 = v2.position - v1.position;
    Vec3f deltaPos3 = v3.position - v1.position;
    Vec2f deltaUv2  = uv2 - uv1;
    Vec2f deltaUv3  = uv3 - uv1;

    float     r         = 1.0 / (deltaUv2[0] * deltaUv3[1] - deltaUv2[1] * deltaUv3[0]);
    deltaPos2 *= deltaUv3[1];
    deltaPos3 *= deltaUv2[1];
    Vec3f tangent = deltaPos2 - deltaPos3;
...
GeometryResource::GeometryResource(ResourceManager* manager, const std::string& name)
    : Resource(manager, name) {}

void GeometryResource::do_load() {
    // Simulate loading geometry data
    std::cout << "Loading geometry: " << get_name() << std::endl;
    loaded_ = true;
}

void GeometryResource::do_unload() {
    // Simulate unloading geometry data
    std::cout << "Unloading geometry: " << get_name() << std::endl;
    loaded_ = false;
}