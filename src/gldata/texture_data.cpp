 ...
 * @brief Implementation of TextureData, a wrapper for 2D/cubemap textures with optional bindless support.
 */
...
void TextureData::set_data(int w,
                           int h,
                           const TextureSpecification& specification,
                           const void* const data[6]) {
    spec   = specification;
    width  = w;
    height = h;
    ...
    if (spec.type == TextureType::TEX_2D) {
        glTexImage2D(target, 0, spec.internal_format, width, height, 0,
                     spec.data_format, spec.data_type, data ? data[0] : nullptr);
    } else if (spec.type == TextureType::TEX_CUBE_MAP) {
        for (unsigned int i = 0; i < 6; ++i) {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, spec.internal_format,
                         width, height, 0, spec.data_format, spec.data_type,
                         data ? data[i] : nullptr);
        }
    }
...
GLuint64 TextureData::get_handle() const {
    return bindless_handle ? bindless_handle : static_cast<GLuint64>(data_id);
}
